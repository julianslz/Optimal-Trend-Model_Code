...
class SpatialContinuityParallel:
    def __init__(self, lag_distance, dataframe, xcoor, ycoor, lag_tol=None, bandh=None, nlag=None,
        ...
        self._lag_tol = self.lag_dist * 2 if lag_tol is None else lag_tol
        self._bandh = self.lag_dist * 2 if bandh is None else bandh
        self._nlag = int(self.max_distance / self.lag_dist) if nlag is None else nlag

        self._square_max_dist = self.max_distance ** 2
        self._bayesian_realizations = None

        self.feat_model = feat_to_model
    ...
    def _semivariograms(self, azimuths):
        ...
        for i, azimuth_i in enumerate(azimuths):
            lag[:, i], gamma[:, i], npp[:, i] = gamv(
                self._dataset, self._xcoor, self._ycoor, self.feat_model, self.tmin, self.tmax,
...
class SpatialContinuitySequential(SpatialContinuityParallel):
    def __init__(self, lag_distance, dataframe, xcoor, ycoor, lag_tol, bandh, nlag, feat_to_model='NFeat'):
        super().__init__(lag_distance, dataframe, xcoor, ycoor, lag_tol, bandh, nlag, feat_to_model)
